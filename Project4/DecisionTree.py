...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    """
    #YOUR CODE HERE
    if self.root == classificationData:
        return self.root
...
def getClassCounts(examples,className):
    ...
    for x in examples:
        ...
        if x[className] not in classCounts.keys():
            classCounts[x[className]] = 1
        else:
            classCounts[x[className]] += 1
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for x in examples:
        val = x[attrName]
        if val in d.keys():
            if x[className] not in d[val].keys():
                d[val][x[className]] = 1
            else:
                d[val][x[className]] += 1
        else:
    ...
    for x in d:
        counts.append(d[x].values())

    if len(counts) < len(attrValues):
        diff = len(attrValues) - len(counts)
...
def remainder(examples,attrName,attrValues,className):
    ...
    toReturn = 0.0
    for x in attrValues:
        atr = getAttributeCounts(examples,attrName,attrValues,className)
        count = 0.0
        for j in atr:
            count += sum(j)

        temp = []
        temp = getPertinentExamples(examples,attrName,x)
        a = len(temp) / count

        val = getClassCounts(temp, className).values()
...
def giniIndex(classCounts):
    ...
    toReturn /= (sum(classCounts) * sum(classCounts))
    toReturn = toReturn + 1
...
def giniGain(examples,attrName,attrValues,className):
    ...
    toReturn = 0.0
    for i in attrValues:
        temp = getPertinentExamples(examples, attrName, i)
        a = len(temp)
        val = getClassCounts(temp, className).values()
        b = giniIndex(val)
